var productModule = function() {
    ...
    var manageSizeStockMessages = function($target) {
        ...
        if ($target.hasClass("out-of-stock")) {
            ...
            $(".stock-message").show().html('<span class="stock-message-text">Your selected size is expected in our warehouse on ' + $target.attr("data-pre-order") + ". You can still order â€“ we will send your item out to you as soon as it arrives.</span>")
        }
    ...
    $(document).ready(function() {
        ...
        $("div.product-mobile-carousel ul.slides").productimages({
            ...
            onImageLoaded: function(container, image) {
                ...
                var newSizeURL = image.url.replace("/250/", "/" + workOutImageSize(false) + "/");
                if (image.ext === "mp4" && !checkURLForString("product/archive")) {
                    ...
                    var $newCutOut = $('<a class="openZoomLink MagicZoomPlus" style="display: block" href="' + image.url.replace("/250/", "/1553/") + '"><img src="' + newSizeURL + '" class="grid-image cut-out" title="Click to zoom" alt="' + tcp_product_env.displayName + '" /></a>');
                    li.append($newCutOut)
                } else {
                    if (!checkURLForString("product/archive")) {
                        li.append('<a class="openZoomLink MagicZoomPlus" style="display: block" href="' + image.url.replace("/250/", "/1553/") + '"><img src="' + newSizeURL + '" class="grid-image" title="Click to zoom" alt="' + tcp_product_env.displayName + '" /></a>')
                    }
                }
                container.append(li);
