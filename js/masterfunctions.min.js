    ...
    if ($pRecs.length > 0) {
        for (var i = $pRecs.length - 1; i >= 0; i--) {
            if (typeof jsonData[0].recs[i] != "undefined") {
                ...
                $($pRecs[i]).find(".p-rec-price").text("Ãƒâ€šÃ‚Â£" + pRecPrice);
        ...
        for (var i = 0; i < jsonData[0].recs.length; i++) {
            if (i < 5) {
                ...
                if (jsonData[0].recs[i].prices.GBP.salePrice == jsonData[0].recs[i].prices.GBP.unitPrice)
                    $ymalLiItem.find("p").text("Ãƒâ€šÃ‚Â£" + jsonData[0].recs[i].prices.GBP.unitPrice);
                else
                    $ymalLiItem.find("p").html('<span class="recommendation-price price-strike-through">Ãƒâ€šÃ‚Â£' + jsonData[0].recs[i].prices.GBP.unitPrice + ' </span><span class="recommendation-sale">Ãƒâ€šÃ‚Â£' + jsonData[0].recs[i].prices.GBP.salePrice + "</span>")
            } else {
                break
            }
        }
    ...
    if ($(".basket-recommendations-container").length == 0 && typeof jsonData[0].recs != "undefined") {
        ...
        for (var i = 0; i < jsonData[0].recs.length; i++) {
            if (i < 5) {
                ...
                var $newItem = $('<img class="grid-image"><span class="basket-recommendation-title">' + jsonData[0].recs[i].title + '</span><span class="basket-recommendation-price">Ãƒâ€šÃ‚Â£' + jsonData[0].recs[i].prices.GBP.unitPrice + "</span>");
                $ymalLiItem.append($newItem);
                if (jsonData[0].recs[i].prices.GBP.unitPrice != jsonData[0].recs[i].prices.GBP.salePrice) {
                    $ymalLiItem.find(".basket-recommendation-price").addClass("price-strike-through");
                    $ymalLiItem.append('<span class="basket-recommendation-sale-price">Ãƒâ€šÃ‚Â£' + jsonData[0].recs[i].prices.GBP.salePrice + "</span>")
                }
            } else {
                break
            }
            ...
            if (tcp_env.country_code.toLowerCase() === "uk") {
                ...
                if (typeof toast_config !== "undefined" && toast_config.in_sale === false) {
                    if (upsellToAdd > 0 && basketValue > 84 && $("#mini-basket-upsell").length === 0 && $("#basket-upsell").length === 0) {
                        $(".mini-basket-sub-total").before("<div id='mini-basket-upsell' style='margin-top:15px; color: #186aa2; text-align: center; text-transform: uppercase; letter-spacing: 1px' class='medFont'>Add Ãƒâ€šÃ‚Â£" + upsellToAdd + " more to your basket for free UK delivery and returns</div>");
                        $("#submitBasket").prepend("<div id='basket-upsell' style='margin-bottom:15px; color: #186aa2; text-align: center; text-transform: uppercase; letter-spacing: 1px' class='medFont grid-100 tablet-grid-100 mobile-grid-100 grid-parent'>Add Ãƒâ€šÃ‚Â£" + upsellToAdd + " more to your basket for free UK delivery and returns</div>");
...
var rangeModule = function() {
    ...
    var setStockMessage = function () {
        ...
        if ($sizeElem.hasClass("out-of-stock")) {
            ...
            $stockMsg.show().html('<span class="message-text">Your selected size is expected in our warehouse on ' + $sizeElem.attr("pre-order") + ". You can still order ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ we will send your item out to you as soon as it arrives.</span>");
    ...
    var displayNewPrice = function(theProduct) {
        if (typeof theProduct !== "undefined") {
            var currency_symbol = tcp_env.currency_symbol === "&#163;" ? "Ãƒâ€šÃ‚Â£" : typeof tcp_env.currency_symbol !== "undefined" ? tcp_env.currency_symbol : "Ãƒâ€šÃ‚Â£";
                        ...
                        if (data.Results[i].ProductStatistics.NativeReviewStatistics.TotalReviewCount !== 0) {
                            ...
                            for (var k = 0; k < keyArr.length; k++) {
                                ...
                                if ($theProd.length > 0) {
                                    ...
                                    $theProd.find(".cat-prod-details").prepend('<span class="review-rating-stars-on review-rating-stars grid-100 tablet-grid-100 mobile-grid-100 grid-parent"><span class="stars-maintain-width"><span class="float-left"><span class="stars-block">ÃƒÂ¢Ã‹Å“Ã¢â‚¬Â¦ÃƒÂ¢Ã‹Å“Ã¢â‚¬Â¦ÃƒÂ¢Ã‹Å“Ã¢â‚¬Â¦ÃƒÂ¢Ã‹Å“Ã¢â‚¬Â¦ÃƒÂ¢Ã‹Å“Ã¢â‚¬Â¦</span></span><span class="number-of-reviews"></span></span></span>');
